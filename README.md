...
Rebuild guard: `make` refuses while a live relay owns port 10555 — run `hush-relay --quit` (Exit) first (Linux; elsewhere Exit in the hive or Ctrl+C); Close is not enough since the hive keeps the port. The guard is port-scoped: it checks `$HUSH_PORT` if set, else `10555`. With curl installed, only a relay answering on that port blocks the build (a relay on another port does not; a pidfile-named owner is refused with its pid and the `--quit` command). Without curl the guard cannot probe, so any running hush-relay blocks the build (exit 2) regardless of port, except a relay the pidfile names as owner of this port, which is refused with exit 1, its pid and the `--quit` command — install curl or quit the other relays first. `make install` does not refuse: it first stops every running `hush-relay*` process you own (renamed copies and any port included; under `sudo`, root's and the invoking user's) and the relay's CHILD turnserver (never the systemd `hush-turn.service` daemon), printing each pid and path, and fails if a relay survives or if `make` itself runs under a relay. `make clean` does the same stop, then closes Hush app windows whose own command line carries `--class=hush-relay*` (a window handed off into your already-running browser is reported and left alone), and removes stale renamed `hush-relay-*` copies from `BINDIR`; `~/.hush` is never touched. `make test` runs the stop checks only with `CI=true` or `HUSH_TEST_STOP_RELAYS=1`, because they stop your relays.
