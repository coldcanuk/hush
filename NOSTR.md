...
export HUSH_PUBKEY_ALLOWLIST=true
...
PGPASSWORD=hush_dev psql -h localhost -U hush -d hush -c \
...
| **Edits (kind:40003)** | ⚠️ | Works on the wire but Hush-only — no standard NIP-29 client renders these |
| **Rich content (kind:40002)** | ⚠️ | Works on the wire but Hush-only — no standard NIP-29 client renders these |
...
When `HUSH_PUBKEY_ALLOWLIST=true`, NIP-42 connections that authenticate with only a pubkey
...
| Kind | Tags | Content |
|------|------|---------|
| **39000** | `d=<uuid>`, `name`, `closed` (always); `about` (if description non-empty); `private` (if applicable); `hidden` (DM channels only) | Group metadata. **Note:** `closed` is always emitted per NIP-29 convention (Hush channels require explicit membership), but open channels are still readable/writable by non-members at runtime. The tag reflects the membership model, not access enforcement. |
...
|--------|----------|:--------:|-------|
| **HushTestClient** | C / harness (repo) | Automated E2E | Full NIP-29 flow: discovery (39000/39001/39002), kind:9 send/receive, reactions, deletions, h-tag enforcement |
...
When `HUSH_REQUIRE_RELAY_MEMBERSHIP=true`, every authenticated connection is checked against the
...
Use `hush-admin` — the operator CLI shipped in the relay image — to manage relay membership.
...
Or invoke `hush-admin` directly inside the container:

```bash
docker compose exec relay hush-admin add-member --pubkey npub1abc...
docker compose exec relay hush-admin add-member --pubkey npub1abc... --role admin
docker compose exec relay hush-admin remove-member --pubkey npub1abc...
docker compose exec relay hush-admin list-members
```
...
| `HUSH_RELAY_PRIVATE_KEY` | Hex private key — required to sign kind:13534 events |
...
|----------|:--------:|---------|-------------|
| `HUSH_PUBKEY_ALLOWLIST` | ❌ | `false` | Enable pubkey allowlist for NIP-42 pubkey-only auth |
| `HUSH_RELAY_PRIVATE_KEY` | ❌ | random | Hex secret key for relay signing (discovery events, system messages) |
| `HUSH_REQUIRE_AUTH_TOKEN` | ❌ | `false` | Require authenticated NIP-42 for all connections |
...
- [`docs/nips/`](docs/nips/) — Hush's own NIP extension documents.
